 ...
 *
 * Disassembly of SSDT1.dat, Thu Apr  6 17:23:31 2023
 *
 * Original Table Header:
 *     Signature        "SSDT"
 *     Length           0x000025E8 (9704)
 *     Revision         0x02
 *     Checksum         0x01
 *     OEM ID           "CpuRef"
...
DefinitionBlock ("", "SSDT", 2, "CpuRef", "CpuSsdt", 0x00003000)
{
    ...
    {
        Name (CTPC, Zero)
        OperationRegion (PNVS, SystemMemory, 0x7F87E000, 0x0065)
    ...
    Scope (\_SB.PR00)
    {
        ...
        Method (GCAP, 1, Serialized)
        {
            ...
            {
                If (((\_SB.ITBS != Zero) && (\_SB.OSCP & 0x1000)))
                {
                    IOB2 = ITBS /* \_SB_.ITBS */
                }
            }
    ...
    Scope (\_SB)
    {
        If ((TXTE == One))
        {
            Device (ACM)
            {
                ...
                {
                    Local0 = (\_SB.CPID & 0xFFFFFFF0)
                    If (((Local0 == 0x000806C0) || (Local0 == 0x000806D0)))
                    {
                        Return ("INT_TGL_SINIT")
                    }

                    If ((Local0 == 0x000A0670))
                    {
                        Return ("INT_RKLS_SINIT")
                    }

                    If (((Local0 == 0x000A0650) && (\_SB.CPID != 0x000A0652)))
                    {
